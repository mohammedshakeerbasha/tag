  ...
  //  echo '<pre>'; print_r($data);exit();
    ...
    public function payment(){
      ...
      if($formdata['billing_type'] == 1){
        ...
        $transId =  $this->auth_payment_transaction_via_card($formdata,$descForTransaction);
        $bt = "CC";
        $formdata['btInt'] = 1;
      }else{
        $descForTransaction = 'Online ACH Payment';
        $transId = $this->achpaymentmanual($formdata);
        $bt = "ACH";
        $formdata['btInt'] = 3;
      }
      // $transId= '44022735689';
      ...
      if(!empty($transId)){
        ...
        $invoice_pay_id = $this->update_invoicepayment_table_on_payment_transaction($formdata,$transId,$invoicePaymentDes);
    ...
    public function update_invoicepayment_table_on_payment_transaction($formdata,$transId,$invoicePaymentDes){
      $payment['pay_client_id'] = $formdata['client_id'];
      $payment['pay_client_account_id'] = $formdata['client_account_id'];
      $payment['apply_pay_invoice_id'] = $formdata['invoice_id'];
      $payment['date_of_payment'] = date("Y-m-d");
