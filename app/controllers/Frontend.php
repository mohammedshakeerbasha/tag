...
class Frontend extends MY_Controller
{
    ...
    public function list_payment_method()
    {
        ...
        $data['client'] = $this->frontend_model->get_client($cid);
    	$id = $this->session->client_userID;
    	$data['title'] = 'Payment Methods';
		$data['client_contact'] = $this->dashboard_model->get_client_contact($id);
		$data['id'] = $id;
		$data['payment_methods'] = $this->client_payment_model->get_client_payment_method_by_clientid( $cid);
    ...
    public function automatic_invoice_payment(){
        ...
        if($this->input->post()){
            $this->frontend_model->automatic_invoice_payment($this->input->post());
        }
    ...
    public function add_payment_method()
    {
		...
		$data['client_contact'] = $this->dashboard_model->get_client_contact($id);
		if($this->input->post()){
		$payment_id = $this->client_payment_model->add_payment_method($this->input->post(),$data['client_contact']);
		if($payment_id){
    ...
    public function delete_payment_method($id){
    	$profile = $this->client_payment_model->get_payment_method($id);
    	include APPPATH.'libraries/Authnetcim.php';
		$cim = new Authnetcim($this->config->item('api_login_id'),$this->config->item('api_transaction_key'),Authnetcim::USE_PRODUCTION_SERVER);
		...
		if ($cim->isSuccessful())
    					{
        				$this->client_payment_model->delete_payment_method($id);
        				$this->session->set_flashdata('payment_method', true);
    					}
    ...
    $this->client_payment_model->make_payment_method_default($id);
    	redirect('frontend/list_payment_method');
    }
     /**
     * App home.
    ...
    public function add_client_address()
    {
        ...
        $cl = $this->frontend_model->add_address($this->input->post(),$id);
        if($cl){
    ...
    public function edit_client_address($address_id)
    {
    	...
    	$address = $this->frontend_model->get_client_address_by_id($address_id);
    	...
    	$nstate = $this->general_model->get_state($this->input->post('state'));
    	 ...
    	 $cid = $this->session->clientID; 
        $client = $this->frontend_model->get_client($cid);
        ...
        $cl = $this->frontend_model->edit_address($this->input->post(),$address_id);
    	if($cl){
      ...
      redirect('dashboard/client_address');
        ...
        $data['back_link']=base_url('/dashboard/client_address');
        $data['states'] = $this->general_model->get_states('230');
        $data['address'] =  $this->frontend_model->get_client_address_by_id($address_id);
   ...
   public function client_test_run($page=1)
    ...
    $etype_num_rows = $this->frontend_model->get_client_account_by_client_id_for_frontend_num_rows_for_frontend($client_id);
       // Generate pagination.
     $perPage = 15;
     // Handle pagination.
     if ($page==1) {
     ...
     $data['offSet'] = $offSet;
     $data['client_accounts'] = $this->frontend_model->get_client_account_by_client_id_for_frontend_with_limit_for_frontend($perPage, $offSet,$client_id); 
    ...
    */
    ...
    {
        $data['title'] = 'Drug Test Details';
        $etype_num_rows = $this->client_test_model->test_selected_by_client_num_rows_frontend($id,$client_account_id);
        // Generate pagination.
        $perPage = 10;
        // Handle pagination.
            if ($page==1) {
        ...
        $data['paginationInfo'] = $this->paginator->newpaginationInfo($etype_num_rows,$perPage);
        // end pagination
        $data['client_account'] = $this->frontend_model->get_client_account($client_account_id);
        $data['client'] = $this->frontend_model->get_client_by_id($data['client_account']->client_id);
        $data['test_runs_details'] = $this->client_test_model->test_selected_by_client_frontend($id,$client_account_id,$perPage, $offSet);
        $data['test_runs_by_id'] = $this->client_test_model->test_runs_by_id($id);
    ...
    */
    ...
    {
        $data['title'] = 'Drug Test Details';
        $etype_num_rows = $this->client_test_model->test_eligible_by_client_num_rows($id,$client_account_id);
        // Generate pagination.
        $perPage = 10;
        // Handle pagination.
            if ($page==1) {
        ...
        $data['client_account'] = $this->frontend_model->get_client_account($client_account_id);
        $data['client'] = $this->frontend_model->get_client_by_id($data['client_account']->client_id);
        $data['test_eligible_runs'] = $this->client_test_model->test_eligible_by_client($id,$client_account_id,$perPage, $offSet);
        $data['test_runs_by_id'] = $this->client_test_model->test_runs_by_id($id);
    ...
    public function export_client_random_pull($test_run_id,$client_account_id){        
         ...
         $data['title']= 'Statement';
          ...
          $this->excel->getActiveSheet()->setTitle('account_statement');                    
                    ...
                    $test_runs_details = $this->client_test_model->test_selected_by_client_pdf($test_run_id,$client_account_id);
         
                    $client = $this->frontend_model->get_client_by_id($id); 
      
                foreach($test_runs_details as $test_runs_detail) { 
                    $emp_count = $this->client_test_model->get_employee_by_run_id_num($test_runs_by_id->id,$test_runs_detail->employee_id);
     ...
     public function download_past_two_years(){
        ...
        $client_accounts = $this->frontend_model->get_client_account_by_client_id_and_custom_date_for_export($this->input->post('from_date'),$this->input->post('to_date'),$client_id);
        }else{  
        $client_accounts=array(); 
        $client_accounts = $this->frontend_model->get_client_account_by_client_id_for_export($client_id); 
        }
    ...
    public function multiple_pdf_exports($client_id,$quarter,$year,$account_id,$client_name,$pull_group_id,$test_run_id,$randompath){ 
                ...
                $data['test_runs_details'] = $this->frontend_model->get_eligible_for_two_years($client_id,$quarter,$year,$account_id,$client_name,$pull_group_id,$test_run_id);       
                $data['test_runs_by_id']   = $this->client_test_model->test_runs_by_id($test_run_id);  
                $data['pull_group']        = $this->general_model->get_pull_group($data['test_runs_by_id']->pull_group_id);
                $data['client_account']    = $this->frontend_model->get_client_account($account_id);
                $data['client']            = $this->frontend_model->get_client_by_id($data['client_account']->client_id);
                $data['client_address']    = $this->frontend_model->get_client_address_by_client_id_pdf($data['client']->id);
                $data['client_contact']    = $this->frontend_model->get_client_by_der_pdf($data['client']->id);
    ...
    public function multiple_exports($client_id,$quarter,$year,$account_id,$client_name,$pull_group_id,$test_run_id,$randompath){
        $test_runs_details = $this->frontend_model->get_eligible_for_two_years($client_id,$quarter,$year,$account_id,$client_name,$pull_group_id,$test_run_id);
    ...
    public function random_selections_download($client_id,$quarter,$year,$account_id,$client_name,$pull_group_id,$test_run_id,$randompath){
       ...
       $data['test_runs_details'] = $this->client_test_model->test_selected_by_client_pdf($test_run_id,$account_id); 
       if(count($data['test_runs_details'])>0){
       $data['test_runs_by_id']   = $this->client_test_model->test_runs_by_id($test_run_id);  
       $data['pull_group']        = $this->general_model->get_pull_group($data['test_runs_by_id']->pull_group_id);
       $data['client_account']    = $this->frontend_model->get_client_account($account_id);
       ...
       $client_accounts_id='';
       $test_runs_details = $this->client_test_model->test_selected_by_client_pdf($test_run_id,$account_id);
       foreach($test_runs_details as $test_runs_detail){ 
       $emp_count= $this->client_test_model->get_employee_by_run_id_num($test_runs_detail->test_run_id,$test_runs_detail->employee_id);
       $employee_cat  =  $this->employees_model->get_employees_categories_by_id($test_runs_detail->emp_cat);
       $employee       =  $this->employees_model->get_employee_by_id($test_runs_detail->employee_id); 
       $this->excel->getActiveSheet()->SetCellValue('A' . $erow, $test_runs_detail->emp_last_name.', '.$test_runs_detail->emp_first_name.' '.$test_runs_detail->emp_middle_initial.'.');
       $this->excel->getActiveSheet()->SetCellValue('B' . $erow, $employee_cat->title);
